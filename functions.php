...
//  wpmail()メール設定
define( 'SMTP_HOST', 'sv****.xserver.jp' );       //メールサーバーのホスト名
...
//  ログインページ変更
//  (指定ファイル名をログインページにするwp-login.php→entrance-login.php等)
...
//
//  YoastSEOなどのプラグイン使用する場合は被るので未使用
//----------------------------------------------------
...
//　
//----------------------------------------------------
//add_action('wp_head', 'my_add_json');
function my_add_json()
{
  //  yoastSEOで出力される内容
  $insert ='';
  $insert.='<script type="application/ld+json"';
  $insert.='class="yoast-schema-graph">';
  $insert.='{"@context":"https://schema.org",';
  $insert.='"@graph":[{"@type":"CollectionPage",';
  $insert.='"@id":"https://",';
  $insert.='"url":"https://",';
  $insert.='"name":"",';
  $insert.='"isPartOf":{"@id":"https:///#website"},';
  $insert.='"description":"",';
  $insert.='"breadcrumb":{"@id":"https:///#breadcrumb"},';
  $insert.='"inLanguage":"ja"},';
  $insert.='{"@type":"BreadcrumbList",';
  $insert.='"@id":"https:///#breadcrumb",';
  $insert.='"itemListElement":[{"@type":"ListItem","position":1,"name":"ホーム"}]},';
  $insert.='{"@type":"WebSite","@id":"https:///#website",';
  $insert.='"url":"https://",';
  $insert.='"name":"SITE NAME",';
  $insert.='"description":"",';
  $insert.='"potentialAction":[{"@type":"SearchAction",';
  $insert.='"target":{"@type":"EntryPoint",';
  $insert.='"urlTemplate":"https:///?s={search_term_string}"},';
  $insert.='"query-input":"required name=search_term_string"}],"inLanguage":"ja"}]}';
  $insert.='</script>';
  echo $insert;
}
...
function admin_favicon() {
  echo '<link rel="shortcut icon" href="'.GET_PATH().'/common/favicon/favicon.ico" type="image/x-icon">';
}
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - --
//  wp_enqueue_styleで出力される不要な属性の消去
//- - - - - - - - - - - - - - - - - - - - - - - - - - --
...
//-----------------------------------------------------
//  ( ↑ wp_enqueue_styleでbody閉じタグ前に入ってしまう事から )
//  CSS読み込み 強制的にhead内で読み込む
//-----------------------------------------------------
function my_head_styles(){
  //  フォント
  echo '    <link rel="preload" href="'.GET_PATH('font').'googlefonts/Noto_Sans_JP/static/NotoSansJP-Medium.woff2" as="font" type="font/woff2" crossorigin>'."\n";
//  echo '    <link rel="preload" href="'.GET_PATH('font').'googlefonts/NotoSerifJP/NotoSerifJP-Regular.woff2" as="font" type="font/woff2" crossorigin>'."\n";
//  echo '    <link rel="preload" href="'.GET_PATH('font').'googlefonts/Lora/static/Lora-Medium.woff2" as="font" type="font/woff2" crossorigin>'."\n";
  //  スタイル
...
function my_enqueue_scripts(){
  if (!is_admin()) {
    ...
    //  headに置く(deferを付ける)ならfalse、footerに置く(deferを付けない)ならtrue
    $fFooter = false;

    //  指定名は全て別にしないと最初の１つしか読み込まれない
    wp_register_script( "head-gsap", GET_PATH('js') . "lib/gsap/gsap.min.js", array(),'1.0', $fFooter );
    wp_register_script( "head-scrolltrigger", GET_PATH('js') . "lib/gsap/ScrollTrigger.min.js", array(),'1.0', $fFooter );
  //    wp_register_script( "head-swiperbundle", GET_PATH('js') . "lib/swiper/swiper-bundle.min.js", array(),'1.0', $fFooter );
    wp_enqueue_script( "head-bundle",
      GET_PATH('js') . "bundle.js?v=".esc_html(date_i18n('Ymd_His')),
      array('head-gsap', 'head-scrolltrigger'),
      '1.0', $fFooter );
    ...
    if (is_page('contact')||is_page('contact-confirm') ) :
      wp_enqueue_script( "head-recaptcha", GET_PATH('js') . "https://www.google.com/recaptcha/api.js?render=".$reCAPTCHA_site_key, false, false );
    endif;
  }
}
add_action( "wp_enqueue_scripts", "my_enqueue_scripts" );

//  cssのfooter化要因の可能性としてこれが挙げられる( いじっている時に一回だけ直った )が、その後コメントしても変化がない。
//  上記scriptの読み込み時に特定名称にdeferを付与
function add_defer($tag, $handle) {
  //  ハンドル名にheadが含まれていないならそのまま返す
  if( strpos( $handle, 'head' ) === false ) {
      return $tag; 
  }
...
add_action("phpmailer_init", function ($phpmailer) {
    ...
    $phpmailer->From       = "***";    //送信者メールアドレス（Gmailの場合は反映されない）
  });
